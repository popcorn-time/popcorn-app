...
var vsprintf = require('sprintf').vsprintf,
    ...
    locales = {},
    localeIds = false,
    api = ['__', '__n', 'getLocale', 'setLocale', 'getCatalog', 'getLocales', 'isValidLocale'],
...
i18n.configure = function i18nConfigure(opt) {
  ...
  // implicitly read all locales if lo opyion defined
  if (typeof opt.locales === 'object')
      opt.locales.forEach(function (l) { read(l); });
  else
      i18n.getLocales().forEach(function (l) { read(l); });

};

// now, i18n automatically obtains the locales based on available language files.
    ...
    if(localeIds === false) {
        ...
        for(var i in files){
            ...
            if(name.indexOf('.json', name.length - 5) !== -1)
                localeIds.push(name.replace('.json', ''));
        }
    }
...
i18n.isValidLocale = function i18nisValidLocale(key){
    return (i18n.getLocales().indexOf(key) !== -1);
};
